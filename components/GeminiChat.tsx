...
import { BoardState, ChatMessage, StoneColor, GameTree, GameNode } from '../types';
...
const GeminiChat: React.FC<GeminiChatProps> = ({
  ...
  const getGtpHistory = (): string[] => {
    ...
    while (ptr && gameTree.nodes[ptr]) {
      const node: GameNode = gameTree.nodes[ptr];
      if (node && node.move) {
        moves.push(toGtpCoordinate(node.move.x, node.move.y));
      }
      ptr = node ? node.parentId : null;
    }
      ...
      <div className="p-3 bg-slate-900 border-b border-slate-800 shrink-0">
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className={`flex items-center justify-center w-9 h-9 rounded-lg border transition-all shrink-0 ${isTtsEnabled
    ...
    <div className="flex flex-col h-full w-full">
      <div className={`flex-1 overflow-y-auto p-4 space-y-4 min-h-0 custom-scrollbar ${minimized ? 'hidden md:block' : 'block'}`}>
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 border ${msg.role === 'user' ? 'bg-indigo-900 border-indigo-700' : 'bg-purple-900 border-purple-700'
                }`}
            >
              {msg.role === 'user' ? <FaUser size={10} className="text-indigo-200" /> : <FaGhost size={12} className="text-purple-200" />}
            </div>
            <div
              className={`max-w-[85%] px-3 py-2.5 rounded-2xl text-[11px] leading-relaxed whitespace-pre-wrap border ${msg.role === 'user'
